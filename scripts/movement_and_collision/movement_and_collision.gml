...
/*Special Note-----------------------------------------------------------------------------------
So it seems like yoyo likes to change how their collision checking works periodically.  
This happened back in GMS1, and I don't understand why it keeps changing.  Regardless, 
if this seems to not be working PERFECTLY try swapping place_meeting for place_meeting_rounded
or visa versa in this script.  Which ever one you currently have in here that doesn't work, swap for the 
other.  Sorry about this, but Yoyo apparently can't make up their damn mind about how
place meeting should work.
------------------------------------------------------------------------------------------------*/

var _movement_direction=argument[0];
var _move_speed=argument[1];
var _wall=argument[2];
var _normalized=true;
var _against_wall=0;
...
if(_movement_direction !=-1 && !place_meeting_rounded(x+lengthdir_x(1,_movement_direction),y+lengthdir_y(1,_movement_direction),_wall))
{   //We are not against a wall.  Move.
    ...
    while(place_meeting_rounded(x,y,_wall))
    {   //We hit a wall.  Back up until we aren't in a wall.
        x-=lengthdir_x(1,_movement_direction);
        y-=lengthdir_y(1,_movement_direction);
        _hit_wall=true;
    }
...
{   //Check for ramps!  Now things are starting to get crazy.
    ...
    var _angle=45;
    var _ramp=false;
    if(!place_meeting_rounded(x+lengthdir_x(1,_movement_direction+_angle),y+lengthdir_y(1,_movement_direction+_angle),_wall))
    {   //Found a ramp counter clockwise!
        _movement_direction+=_angle;
        _ramp=true;
    }
    else if(!place_meeting_rounded(x+lengthdir_x(1,_movement_direction-_angle),y+lengthdir_y(1,_movement_direction-_angle),_wall))
    {   //Then clockwise
        _movement_direction-=_angle;
        _ramp=true;
    }
